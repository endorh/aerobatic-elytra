package dnj.aerobatic_elytra.client.trail;

import dnj.aerobatic_elytra.client.trail.BoostShape.BurstBoostShape;
import dnj.aerobatic_elytra.client.trail.BoostShape.CircleBoostShape;
import dnj.aerobatic_elytra.client.trail.BoostShape.ShapedBoostShape;
...
import dnj.aerobatic_elytra.common.particle.TrailParticleData;
...
import static dnj.aerobatic_elytra.common.capability.IElytraSpec.RocketExplosion.*;
import static dnj.aerobatic_elytra.common.item.IAbility.Ability.TRAIL;
...
public class AerobaticTrail {
	...
	public static Optional<TrailParticleData> getTrailParticle(
	  ...
	  int partial, float partialTick, boolean ownPlayer, Vec3f rollVec
		...
		final int life = round(getLife(explosion.type, explosion.trail) * spec.getAbility(TRAIL));
		final float size = (float) clampedLerp(0.4F, 0.5F, spec.getAbility(TRAIL));
	...
	public static TrailParticleData getBoostParticle(
	  LivingEntity player, RocketExplosion explosion, float trailMod
		...
		final float size = (float) clampedLerp(0.4F, 0.5F, trailMod);
	...
	public enum RocketSide {
		LEFT("Left", "aerobatic-elytra.side.left", WingSide.LEFT),
		RIGHT("Right", "aerobatic-elytra.side.right", WingSide.RIGHT),
		CENTER_LEFT("CenterLeft", "aerobatic-elytra.side.center_left", WingSide.LEFT),
		CENTER_RIGHT("CenterRight", "aerobatic-elytra.side.center_right", WingSide.RIGHT);
