...
import dnj.aerobatic_elytra.client.render.model.AerobaticElytraChestModel;
import dnj.aerobatic_elytra.client.render.model.AerobaticElytraModel;
...
import static dnj.aerobatic_elytra.client.ModResources.TEXTURE_AEROBATIC_ELYTRA;
import static dnj.aerobatic_elytra.client.ModResources.TEXTURE_BACK;
	...
	@Override public void render(
	  ...
	  float partialTicks, float ageInTicks, float netHeadYaw, float headPitch
		...
		mStack.push(); {
			...
			final boolean effect = item.hasModelEffect(elytra)
			                       && ClientConfig.aerobatic_elytra_wing_glint_alpha > 0F;
			if (effect) {
				...
				modelElytra.renderGlint(
				  ...
				  1F, 1F, 1F, ClientConfig.aerobatic_elytra_wing_glint_alpha);
			}
			...
			for (WingSide side : WingSide.values()) {
				...
				if (wingDyement.hasPattern) {
					renderWingBanner(
					  mStack, buffer, packedLight, entity, limbSwing, limbSwingAmount, partialTicks,
					  ageInTicks, netHeadYaw, headPitch, elytra, side, item,
					  wingDyement.patternColorData);
				} else {
					renderWingDyed(
					  mStack, buffer, packedLight, entity, limbSwing, limbSwingAmount, partialTicks,
	...
	public void renderBackRockets(
	  ...
	  float netHeadYaw, float headPitch, ItemStack elytra
		...
		IVertexBuilder backBuilder = ItemRenderer.getArmorVertexBuilder(
		  buffer, RenderType.getArmorCutoutNoCull(TEXTURE_BACK), false, item.hasModelEffect(elytra));
