...
import dnj.aerobatic_elytra.network.WeatherPackets.SWindNodePacket;
		...
		protected void tick() {
			...
			if (rain > 0F || !wind.isZero(1E-15)) {
				float wind_randomness =
				  Config.rain_wind_randomness_per_tick * rain * Config.rain_wind_strength_per_tick
				  + Config.storm_wind_randomness_per_tick * storm * Config.storm_wind_strength_per_tick;
				if (wind_randomness > 0F) {
					float wind = rain * Config.rain_wind_strength_per_tick + storm * Config.storm_wind_strength_per_tick;
					windDelta.setRandom(wind_randomness);
					this.wind.add(windDelta);
					this.wind.clamp(wind);
					angularWindDelta.setRandom(
					  Config.rain_wind_randomness_per_tick * rain *
					  Config.rain_wind_angular_strength_per_tick
					  + Config.storm_wind_randomness_per_tick * storm *
					    Config.storm_wind_angular_strength_per_tick
					);
					angularWind.add(angularWindDelta);
					angularWind.clamp(
					  rain * Config.rain_wind_angular_strength_per_tick
					  + storm * Config.storm_wind_angular_strength_per_tick);
				}
