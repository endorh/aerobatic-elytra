...
import endorh.aerobatic_elytra.client.config.ClientConfig.style.dark_theme;
import endorh.aerobatic_elytra.integration.jei.AerobaticElytraJeiHelper;
import endorh.aerobatic_elytra.integration.jei.AerobaticElytraJeiPlugin;
...
import static endorh.aerobatic_elytra.integration.jei.gui.ShapelessDecoratedDrawable.shapelessIconHeight;
import static endorh.aerobatic_elytra.integration.jei.gui.ShapelessDecoratedDrawable.shapelessIconWidth;
...
public abstract class BaseCategory<T> implements IRecipeCategory<T> {
	...
	public BaseCategory(
	  ...
	  Item icon, Item iconSecond, boolean shapeless
		...
		localizedNameKey = "aerobatic-elytra.recipe.category." + shortName;
	...
	@Override public @NotNull List<ITextComponent> getTooltipStrings(
	  @NotNull T recipe, double mouseX, double mouseY
		...
		if (shapeless && inRect(
		  mouseX, mouseY, background.getWidth() - shapelessIconWidth(),
		  0, shapelessIconWidth(), shapelessIconHeight()))
