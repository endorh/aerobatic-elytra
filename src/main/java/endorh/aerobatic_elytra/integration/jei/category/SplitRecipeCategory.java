...
import endorh.aerobatic_elytra.common.recipe.SplitRecipe.LeaveData;
...
public class SplitRecipeCategory extends BaseCategory<SplitRecipe> {
	public static final ResourceLocation UID = prefix("split");
	...
	@Override public @NotNull List<ITextComponent> getTooltipStrings(
	  @NotNull SplitRecipe recipe, double mouseX, double mouseY
		...
		if (inRect(mouseX, mouseY, 61, 19, 22, 15))
			tt.addAll(optSplitTtc("aerobatic-elytra.jei.help.category.split"));
		else if (inRect(mouseX, mouseY, 7, 60, 17, 19))
			tt.addAll(optSplitTtc("aerobatic-elytra.jei.help.category.split.remainder"));
		return tt;
	}
}
