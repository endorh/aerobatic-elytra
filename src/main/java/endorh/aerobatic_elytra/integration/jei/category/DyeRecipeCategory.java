...
import endorh.aerobatic_elytra.common.recipe.DyeRecipe;
...
public class DyeRecipeCategory extends BaseCategory<DyeRecipeCategory.DyeRecipeWrapper> {
	public static final ResourceLocation UID = prefix("dye");
	...
	@Override public void setRecipe(
	  @NotNull IRecipeLayout layout, @NotNull DyeRecipeWrapper recipe, @NotNull IIngredients ingredients
	) {
	...
	public static List<ItemStack> dye(List<ItemStack> stacks, List<List<DyeItem>> dyes) {
		if (dyes.isEmpty())
			return stacks;
		return IntStream.range(0, AerobaticElytraJeiHelper.mcm(stacks.size(), dyes.get(0).size())).mapToObj(i ->
	...
	@Override public @NotNull List<ITextComponent> getTooltipStrings(
	  @NotNull DyeRecipeWrapper recipe, double mouseX, double mouseY
		...
		if (inRect(mouseX, mouseY, 61, 19, 22, 15))
			tt.addAll(optSplitTtc("aerobatic-elytra.jei.help.category.dye"));
		return tt;
	}
